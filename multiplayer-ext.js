...
(() => {
  ...
  function placeStone(row, col, color) {
    ...
    return { success: true, captured: totalCaptures, capturedGroup: capturedList };
  }
  ...
  function showRoomUI(code, inviteLink) {
    ...
    if (!panel) {
      ...
      panel.style.cssText = `
        position: fixed; top: 60px; right: 16px; z-index: 950;
        background: rgba(16,24,32,0.92); border: 1px solid rgba(255,255,255,0.12);
        border-radius: 14px; padding: 14px 18px; color: #eef4fb;
    ...
    panel.innerHTML = `
      <div style="font-weight:bold;margin-bottom:8px;">🔗 房间: <span style="color:#f6c453;">${code}</span></div>
      <div style="font-size:0.75rem;color:rgba(238,244,251,0.7);margin-bottom:10px;">
      ...
      <div id="mp-copy-toast" style="
        text-align:center;font-size:0.72rem;color:#4caf50;margin-top:6px;display:none;
      ">✅ 已复制!</div>
    `;
